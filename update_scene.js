...
const new_block = `      {/* SCENES 2, 3, 4: SINGOLE BIRRE */}
      {BEERS.map((b, i) => {
        const sceneIdx = i + 2;
        return (
...
const startIdx = content.indexOf("      {/* SCENES 2, 3, 4: SINGOLE BIRRE */}");
const endIdx = content.indexOf("      {/* SCENE 5: SCEGLI */}");

if (startIdx !== -1 && endIdx !== -1) {
    const newContent = content.slice(0, startIdx) + new_block + "\n\n" + content.slice(endIdx);
    fs.writeFileSync('src/components/OurBeers.tsx', newContent);
    console.log("Success");
} else {
